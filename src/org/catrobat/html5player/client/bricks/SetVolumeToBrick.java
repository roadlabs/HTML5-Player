...
public class SetVolumeToBrick extends Brick {
  ...
  public SetVolumeToBrick(String sprite, Formula volume) {
    super(sprite);
    this.volume = volume;
  }
  ...
  public boolean execute(Sprite sprite) {
    double calculatedVolume = volume.interpretFloat(sprite);
    if (calculatedVolume < 0.0) {
      calculatedVolume = 0.0;
    } else if (calculatedVolume > 100.0) {
      calculatedVolume = 100.0;
    }
    sprite.setVolume(calculatedVolume);
    return true;
  }
